<template>
	<view class="wrap">
		<view class="top">
			...
			<uni-data-picker placeholder="请选择所在地区"
				popup-title="请选择城市"
		...
		<view class="bottom">
			...
			<view v-if="!change">
				<u-button type="primary"
					class="btn"
					@click="add">确认并提交</u-button>
			</view>
			<view class="btn_grow"
				v-else>
				<u-button type="primary"
					class="btn"
					@click="dataModify">确认修改</u-button>
				<view class="btn-delete">
					<u-button type="warning "
						class="btn "
						@click="dataDelete">删除地址</u-button>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async dataDelete() {
				await addressDelete(this.data).then(() => {
					this.$u.toast('删除成功')
					setTimeout(() => {
						this.$u.route({
							type: 'reLaunch',
