	...
	<view class="previewPage">
		<view class="u-p-t-10">
			...
			<view class="container">
				<view v-for="(item,index) in shoplist"
					:key="index"
					class="cartGoods flexSpace">
					<view class="flex">
						<image style="width: 250rpx;"
							mode='widthFix'
							:src="item.goods.cover1" />
						<view class="flexColumn" style="justify-content: space-around;margin-left: 15rpx;">
							<view class="ellipsis" style="width: 400rpx;">{{item.goods.title}}</view>
							<view style="color: red;">{{item.goods.price}}</view>
						</view>
					</view>
					<view style="position: relative;right: 15rpx;">
						x{{item.num}}
					</view>
				</view>
			</view>
			<view class="footer">
				<view class="tp">
					...
					<u-button ref="submit"
						style="padding: 0 100rpx;line-height: 50px;margin: 0;margin-right: 40rpx;"
			...
			async cartPreview() {
				...
				if (data != undefined) {
					...
					this.location = addressData.address.province + "-" + addressData.address.city + "-" + addressData
						.address.county + "-" + addressData.address.address
